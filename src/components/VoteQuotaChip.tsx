...
// 1時間あたりのいいね/bad票の残り。スーパーいいね(😍)はいいね2票分。
export function VoteQuotaLabel({ light = false, compact = false }: { light?: boolean; compact?: boolean }) {
  ...
  const hint = [
    "1時間ごとに使った票が順に回復します。スーパーいいね(😍)はいいね2票分です。",
    quota.likeResetSeconds > 0 ? `いいね: あと${resetMinutes(quota.likeResetSeconds)}分で1票回復` : "",
    quota.badResetSeconds > 0 ? `bad: あと${resetMinutes(quota.badResetSeconds)}分で1票回復` : "",
  ]
    .filter(Boolean)
    .join(" / ");
  return (
